//NOTE:Function to show alert messages
function showAlert(message, className) {
    ...
    setTimeout(() => {
        if (div.parentElement) {
            div.remove(); // Remove the div after 7 seconds
        }
    }, 9000);
}
...
//NOTE: Function to update student numbers
...
//NOTE:Function to handle add/update button clicks
function handleAddUpdate() {
    ...
    if (!isValid) {
        showAlert(`Please fill in all input fields: ${missingFields.join(', ')}`, "warning",);
        return;
    }

    if (isEmailDuplicate(values.email) && !editingRow) {
        showAlert("Email is already exist", "danger");
    ...
    if (editingRow) {
        // NOTE:Update the existing row with new values
        ...
        // NOTE:Show confirmation modal for update
...
//NOTE:Input validation for the name field
...
//NOTE:Input validation for the email field
inputFields.email.addEventListener("input", (event) => {
    ...
    // Allow two dots in the email
    let dotCount = 0;
    inputValue = inputValue.replace(/\./g, (match, index) => {
        if (index === inputValue.indexOf(".")) {
            dotCount++;
            return match;
        } else if (dotCount < 2) {
            dotCount++;
    ...
    //NOTE: write 2 character sin ban write @ kert 
    ...
    //NOTE: write 2 characters sin ban use . ban
    if (inputValue.includes(".")) {
        const atIndex = inputValue.indexOf(".");
        if (atIndex < 2) {
            inputValue = inputValue.slice(0, atIndex) + inputValue.slice(atIndex + 1);
...
//NOTE:Input validation for the age field
...
//NOTE:Function to check for duplicate emails
